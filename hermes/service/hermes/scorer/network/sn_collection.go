...
func (nodes *SuperNodeList) AddNewNode(client node.ClientInterface, txid string, address string, pastelID string, idx int, nodeMaker node.RealNodeMaker) {
	someNode := NewSuperNode(client, txid, address, pastelID, idx, nodeMaker)
