	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("testCase-%d", i), func(t *testing.T) {
			...
			for _, a := range testCase.nodes {
				...
				client.ListenOnRegisterArtworkProbeImage(testCase.fingersAndScore, testCase.err)
			...
			for _, client := range clients {
				...
				client.AssertRegisterArtworkProbeImageCall(testCase.numberProbeImageCall, testCase.args.ctx, testCase.args.file)
			}
		})
	}
}
