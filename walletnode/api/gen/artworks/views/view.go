...
func ValidateTaskViewTiny(result *TaskView) (err error) {
	...
	if result.Status != nil {
		if !(*result.Status == "Task Started" || *result.Status == "Connected" || *result.Status == "Image Probed" || *result.Status == "Image And Thumbnail Uploaded" || *result.Status == "Status Gen ReptorQ Symbols" || *result.Status == "Preburn Registration Fee" || *result.Status == "Ticket Accepted" || *result.Status == "Ticket Registered" || *result.Status == "Ticket Activated" || *result.Status == "Error Insufficient Fee" || *result.Status == "Error Signatures Dont Match" || *result.Status == "Error Fingerprints Dont Match" || *result.Status == "Error ThumbnailHashes Dont Match" || *result.Status == "Error GenRaptorQ Symbols Failed" || *result.Status == "Task Rejected" || *result.Status == "Task Completed") {
