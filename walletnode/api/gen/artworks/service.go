...
type Service interface {
	...
	// Search artwork request
	Search(context.Context, *SearchPayload, SearchServerStream) (err error)
}
...
// MethodKey key.
var MethodNames = [6]string{"register", "registerTaskState", "registerTask", "registerTasks", "uploadImage", "search"}
...
// SearchServerStream is the interface a "search" endpoint server stream must
// satisfy.
type SearchServerStream interface {
	// Send streams instances of "ArtworkTicket".
	Send(*ArtworkTicket) error
	// Close closes the stream.
	Close() error
}

// SearchClientStream is the interface a "search" endpoint client stream must
// satisfy.
type SearchClientStream interface {
	// Recv reads instances of "ArtworkTicket" from the stream.
	Recv() (*ArtworkTicket, error)
}
...
// SearchPayload is the payload type of the artworks service search method.
type SearchPayload struct {
	// artwork search request
	Term *string
}

// ArtworkTicket is the result type of the artworks service search method.
