...
// SearchEndpointInput holds both the payload and the server stream of the
// "search" method.
type SearchEndpointInput struct {
	// Payload is the method payload.
	Payload *SearchPayload
	// Stream is the server stream used by the "search" method to send data.
...
// "search" of service "artworks".
func NewSearchEndpoint(s Service) goa.Endpoint {
	return func(ctx context.Context, req interface{}) (interface{}, error) {
		ep := req.(*SearchEndpointInput)
		return nil, s.Search(ctx, ep.Payload, ep.Stream)
	}
}
