...
// set to call the "artworks" service "search" endpoint
func (c *Client) BuildSearchRequest(ctx context.Context, v interface{}) (*http.Request, error) {
	var (
		term string
	)
	{
		p, ok := v.(*artworks.SearchPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("artworks", "search", "*artworks.SearchPayload", v)
		}
		if p.Term != nil {
			term = *p.Term
		}
	...
	u := &url.URL{Scheme: scheme, Host: c.host, Path: SearchArtworksPath(term)}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("artworks", "search", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// artworks search endpoint. restoreBody controls whether the response body
// should be restored after having been read.
...
func DecodeSearchResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (interface{}, error) {
	return func(resp *http.Response) (interface{}, error) {
		...
		case http.StatusOK:
			...
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("artworks", "search", err)
			}
			err = ValidateSearchResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("artworks", "search", err)
			}
		...
		case http.StatusInternalServerError:
			...
			err = ValidateSearchInternalServerErrorResponseBody(&body)
			if err != nil {
				return nil, goahttp.ErrValidationError("artworks", "search", err)
			}
			return nil, NewSearchInternalServerError(&body)
		default:
			body, _ := ioutil.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("artworks", "search", resp.StatusCode, string(body))
		}
	}
}
