...
// Search returns an endpoint that makes HTTP requests to the artworks service
// search server.
func (c *Client) Search() goa.Endpoint {
	...
	return func(ctx context.Context, v interface{}) (interface{}, error) {
		...
		if err != nil {
			...
			return nil, goahttp.ErrRequestError("artworks", "search", err)
		}
		if c.configurer.SearchFn != nil {
			conn = c.configurer.SearchFn(conn, cancel)
		}
		go func() {
			<-ctx.Done()
			conn.WriteControl(
		...
		stream := &SearchClientStream{conn: conn}
		return stream, nil
	}
}
