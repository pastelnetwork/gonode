...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"Search", "GET", "/artworks/search/{term}"},
		...
		Search:            NewSearchHandler(e.Search, mux, decoder, encoder, errhandler, formatter, upgrader, configurer.SearchFn),
		CORS:              NewCORSHandler(),
	}
}
...
// MountSearchHandler configures the mux to serve the "artworks" service
// "search" endpoint.
...
// calls the "artworks" service "search" endpoint.
func NewSearchHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var cancel context.CancelFunc
		ctx, cancel = context.WithCancel(ctx)
		v := &artworks.SearchEndpointInput{
			Stream: &SearchServerStream{
				upgrader:   upgrader,
				configurer: configurer,
				cancel:     cancel,
				w:          w,
			...
			Payload: payload.(*artworks.SearchPayload),
		}
		_, err = endpoint(ctx, v)
		if err != nil {
			if _, ok := err.(websocket.HandshakeError); ok {
