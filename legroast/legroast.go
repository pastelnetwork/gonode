...
#cgo amd64 LDFLAGS: -L./lib -llegroast-win
#include "sign.h"
*/
import "C"
import (
...
func Sign(pk []byte, sk []byte, message []byte) []byte {
	...
	var signal_len C.ulonglong

	C.sign((*C.uchar)(&sk[0]), (*C.uchar)(&pk[0]), (*C.uchar)(&message[0]), (C.ulonglong)(len(message)), (*C.uchar)(&signal[0]), (*C.ulonglong)(&signal_len))
...
func Verify(pk []byte, message []byte, signal []byte) int {
	return (int)(C.verify((*C.uchar)(&pk[0]), (*C.uchar)(&message[0]), (C.ulonglong)(len(message)), (*C.uchar)(&signal[0])))
}
