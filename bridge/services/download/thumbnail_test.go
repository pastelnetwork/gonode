...
func TestFetch(t *testing.T) {
	...
	for name, tc := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			nodeClientMock.ListenOnConnect("", nil).ListenOnDownloadThumbnail(tc.want, nil).ListenOnClose(nil)
			...
			err := thumbnail.Connect(ctx, 1, cancel)
			assert.Nil(t, err)

			data, err := thumbnail.Fetch(ctx, "txid", 1)
			assert.Nil(t, err)
			assert.Equal(t, data[0], tc.want[0])
		})
	}
}
