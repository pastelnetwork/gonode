...
func (n *NodeList) AddFirst(node *Node) {
	n.Mux.Lock()         // lock for writing
	defer n.Mux.Unlock() // ensure the lock is released no matter what

	// Prepend the node to the slice
	n.Nodes = append([]*Node{node}, n.Nodes...)
}
